...
class StakingContract extends IContract {
   ...
   * @return {Promise<{createdAt: Date, APR: number, endDate: Date, subscribers: Address[], subscriptionIds: number[], individualMinimumAmount: number, currentAmount: number, _id: number, totalMaxAmount: number, lockedUntilFinalization: boolean, startDate: Date}>}
   */
   ...
   * @param {number} params.product_id
   * @return {Promise<{finalized: boolean, amount: number, APR: number, productId: number, endDate: Date, _id: number, withdrawAmount: number, subscriberAddress: Address, startDate: Date}>}
   */
