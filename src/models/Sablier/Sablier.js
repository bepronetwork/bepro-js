...
export default class Sablier extends IContract {
  ...
  async updateFee({ feePercentage }) {
    // const fee = Numbers.toSmartContractDecimals(feePercentage, 2);
  ...
  async balanceOf({ streamId, who }) {
    ...
    const ret = Numbers.fromDecimalsToBN(
    // const ret = Numbers.fromTokens(
      balance,
      decimals,
    );
    console.log('Sablier.balanceOf: raw ', balance, ' converted ', ret);
    return ret;
  }
  ...
  async getEarnings({ tokenAddress }) {
    ...
    /* const ret2 = Numbers.fromExponential(
      earnings,
    );
    const ret3 = Numbers.fromTokens(
      earnings,
    decimals,
    ); */
    console.log('...Sablier.getEarnings.earnings: ', earnings, ' | decimals: ', decimals);
    console.log('...Sablier.getEarnings.earnings.fromDecimalsToBN: ', ret1);
    // console.log('...Sablier.getEarnings.earnings.fromExponential: ', ret2);
    // console.log('...Sablier.getEarnings.earnings.fromTokens: ', ret3);
    return ret1;
  }
  ...
  async createStream({
    ...
    const decimals = await this.getWeb3Contract().methods.getTokenDecimals(tokenAddress).call();
    const depositWithDecimals = Numbers.fromBNToDecimals(
      deposit,
      decimals,
    );
    // const startTime2 = Numbers.timeToSmartContractTime(startTime);
    // const stopTime2 = Numbers.timeToSmartContractTime(stopTime);
  ...
  async createCompoundingStream({
    ...
    // const startTime2 = Numbers.timeToSmartContractTime(startTime);
    // const stopTime2 = Numbers.timeToSmartContractTime(stopTime);
    // console.log('Sablier.createCompoundingStream.bp3.startTime | startTime2: ', startTime, ' | ', startTime2);
    // console.log('Sablier.createCompoundingStream.bp3.stopTime  | stopTime2 : ', stopTime, ' | ', stopTime2);
