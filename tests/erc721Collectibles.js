...
context('ERC721 Collectibles', async () => {
  let erc721Contract; let
    erc20Contract;
  let app;
  let tokensHeld; let subscriptionId; let withdrawTx; let startDateSubscription; let
    endDateSubscription;
  ...
  it('should add base tokenURI', mochaAsync(async () => {
    const res = await erc721Contract.setBaseTokenURI({ URI: 'https://bepronetwork.github.io/B.E.P.R.O/meta/' });
    expect(res).to.not.equal(false);
  }));
  ...
  it('should open a pack', mochaAsync(async () => {
    /* Approve */
    await erc721Contract.approveERC20();
    const isApproved = await erc721Contract.isApproved({ address: app.account.getAddress(), amount: 1000 });
