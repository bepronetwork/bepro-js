...
contract(TEST_CONTRACT_NAME, async (accounts) => {
  ...
  it(`${TEST_TAG}should set max tokens to lock per user`, async () => {
    ...
    assert.equal(
      res.logs[0].event,
      'MaxAmountToLockChanged',
      'should be the "MaxAmountToLockChanged" event',
    );
    assert.equal(
      res.logs[0].args.admin,
      owner,
      'logs the admin address that performed the operation',
    );
    assert.equal(res.logs[0].args.oldValue, 0, 'logs the old value');
    assert.equal(
      res.logs[0].args.newValue,
      maxTokenAmount,
      'logs the new value',
    );
  ...
  it(`${TEST_TAG}should set min tokens to lock per user`, async () => {
    ...
    assert.equal(
      res.logs[0].event,
      'MinAmountToLockChanged',
      'should be the "MinAmountToLockChanged" event',
    );
    assert.equal(
      res.logs[0].args.admin,
      owner,
  ...
  it(
    `${TEST_TAG}only Admin should set max tokens to lock per user`,
    async () => {
      // make sure we try to set valid amount different than current value
      const maxTokenAmount2 = 7000 + 1;
      ...
      try {
        ...
        assert(
          ...
          'user setMaxAmountToLock should fail',
        );
      }
  ...
  it(
    `${TEST_TAG}only Admin should set min tokens to lock per user`,
    async () => {
      // make sure we try to set valid amount different than current value
      const minTokenAmount2 = 1000 - 1;
      ...
      try {
        ...
        assert(
          error.message.indexOf('revert') >= 0,
          'user setMinAmountToLock should fail',
        );
      }
  ...
  it(`${TEST_TAG}user should lock tokens`, async () => {
    // Approve Tx
    const totalSupply = await erc20Test.totalSupply();
    ...
    expect(
      (await erc20Test.allowance(owner, erc20Lock.address)).toString(),
    ...
    assert.equal(
      res.logs[0].event,
      'TokensLocked',
      'should be the "TokensLocked" event',
    );
    assert.equal(
      res.logs[0].args.user,
      userAddress,
      'logs the locked tokens owner address',
    );
    // assert.equal(res.logs[0].args.startDate, ..., 'logs the startDate of locked tokens event');
    assert.equal(
      res.logs[0].args.endDate,
      endDate,
      'logs the endDate of locked tokens event',
    );
  ...
  it(
    `${TEST_TAG}user should fail to release tokens before release date`,
    async () => {
      const userAddress = owner;
      ...
      try {
        ...
        assert(
          error.message.indexOf('revert') >= 0,
          'user should fail to release tokens',
        );
      }
    },
  );
  ...
  it(
    `${TEST_TAG}user should release and withdraw unlocked tokens`,
    async () => {
      ...
      setTimeout(async () => {
        ...
        assert.equal(
          res.logs[0].event,
          'TokensReleased',
          'should be the "TokensReleased" event',
        );
        assert.equal(
          res.logs[0].args.user,
          userAddress,
          'logs the released tokens owner address',
        );
        assert.equal(
          res.logs[0].args.amount,
          lockTokens,
          'logs the released tokens amount',
        );
