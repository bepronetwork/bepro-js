...
const timeDiff = Numbers.timeToSmartContractTime(endDate) - Numbers.timeToSmartContractTime(startDate);
...
context('Staking Contract', async () => {
  let stakingContract;
  let app;
  let productId; let subscriptionId; let withdrawTx; let startDateSubscription; let
    endDateSubscription;
  ...
  it('should deploy Staking Contract', mochaAsync(async () => {
    /* Create Contract */
    stakingContract = app.getStakingContract({ tokenAddress });
    /* Deploy */
  ...
  it.skip('should create a Product', mochaAsync(async () => {
    /* Create Event */
  ...
  it.skip('should get Product Data', mochaAsync(async () => {
    /* Create Event */
  ...
  it.skip('should get APR Data', mochaAsync(async () => {
  ...
  it.skip('should get Held Tokens == 0', mochaAsync(async () => {
    /* Create Event */
    const res = await stakingContract.heldTokens();
    expect(Numbers.fromExponential(res).toString()).to.equal(Number(0).toString());
  }));

  it.skip('should get Available Tokens == 0', mochaAsync(async () => {
    /* Create Event */
    const res = await stakingContract.availableTokens();
    expect(Numbers.fromExponential(res).toString()).to.equal(Number(0).toString());
  }));

  it.skip('should get Future Locked Tokens == 0', mochaAsync(async () => {
    const res = await stakingContract.futureLockedTokens();
    expect(Numbers.fromExponential(res).toString()).to.equal(Number(0).toString());
  }));

  it.skip('should get tokens needed for APR == totalNeededAPR', mochaAsync(async () => {
  ...
  it.skip('should fund with tokens needed for APR', mochaAsync(async () => {
    const neededTokensAmount = await stakingContract.getTotalNeededTokensForAPRbyAdmin();
    const res = await stakingContract.depositAPRTokensByAdmin({ amount: neededTokensAmount });
    expect(res).to.not.equal(false);
  }));

  it.skip('should get Held Tokens == APR Needed for 1 subscription with min Amount', mochaAsync(async () => {
    const res = await stakingContract.heldTokens();
    const tokensNeeded = await stakingContract.getTotalNeededTokensForAPRbyAdmin();
    expect(Numbers.fromExponential(res).toString()).to.equal(tokensNeeded);
  }));

  it.skip('should get Available Tokens == APR Needed for 1 subscription with min Amount', mochaAsync(async () => {
    const res = await stakingContract.availableTokens();
    const tokensNeeded = await stakingContract.getTotalNeededTokensForAPRbyAdmin();
    expect(Numbers.fromExponential(res).toString()).to.equal(tokensNeeded);
  }));
  ...
  it.skip('should get subscribe to product Data & APR Right', mochaAsync(async () => {
    /* Approve Tx */
  ...
  it.skip('should get Subscription Data Right', mochaAsync(async () => {
  ...
  it.skip('should get Held Tokens == APR Amount + indivualAmount', mochaAsync(async () => {
    const res = await stakingContract.heldTokens();
    expect(res).to.equal(Number(individualMinimumAmount + totalNeededAPR).toString());
  }));

  it.skip('should get Future Locked Tokens == APR Amount', mochaAsync(async () => {
    const res = await stakingContract.futureLockedTokens();
    const userAPR = APR / 365 / 24 / 60 * (Numbers.timeToSmartContractTime(endDateSubscription) - Numbers.timeToSmartContractTime(startDateSubscription)) / 60 * individualMinimumAmount / 100;
    expect(res).to.equal(Number(individualMinimumAmount + userAPR).toString());
  }));

  it.skip('should get Available Tokens == 0 (all used)', mochaAsync(async () => {
    const res = await stakingContract.availableTokens();
    const userAPR = APR / 365 / 24 / 60 * (Numbers.timeToSmartContractTime(endDateSubscription) - Numbers.timeToSmartContractTime(startDateSubscription)) / 60 * individualMinimumAmount / 100;
    expect(res).to.equal(Number(totalNeededAPR - userAPR).toFixed(18));
  }));

  it.skip('should withdraw Subscription', mochaAsync(async () => {
    setTimeout(async () => {
  ...
  it.skip('should confirm Subscription Data after Withdraw', mochaAsync(async () => {
    const res = await stakingContract.getSubscription({
      subscription_id: subscriptionId,
      product_id: productId,
    });
