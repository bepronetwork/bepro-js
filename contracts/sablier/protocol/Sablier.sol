...
contract Sablier is IERC1620, Ownable, PausableWithoutRenounce, Exponential, ReentrancyGuard {
     ...
     * @return sender recipient deposit tokenAddress startTime stopTime remainingBalance ratePerSecond The stream object.
     */
     ...
     * @return sender recipient deposit tokenAddress startTime stopTime remainingBalance ratePerSecond exchangeRateInitial senderSharePercentage recipientSharePercentage The compounding stream object.
     */
     ...
     * @return senderInterest recipientInterest sablierInterest The interest accrued by the sender, the recipient and sablier, respectively, as uint256s.
     */
	 ...
	 * @param amount Withdraw tokens amount.
     */
	...
	/**
     * @notice Get token decimals given a stream id.
     * @dev This is an utility function for DApp layer when converting to float numbers.
     * @param streamId The id of the stream.
	 */
    function getTokenDecimalsFromStream(uint256 streamId)
		external
		streamExists(streamId)
        returns (uint8)
	{
        Types.Stream memory stream = streams[streamId];
        return IERC20View(stream.tokenAddress).decimals();
    }
